...
export const SettingsPage: React.FC = () => {
  ...
  const fetchHealth = useCallback(async () => {
    ...
    try {
      ...
      if (generation === requestGeneration.current) setHealth(res);
    } catch (err: any) {
  ...
  const invalidateHealthRequest = useCallback(() => { ++requestGeneration.current; }, []);
  useEffect(() => {
    void fetchHealth();
    return invalidateHealthRequest;
  }, [fetchHealth, invalidateHealthRequest]);
        ...
        <Card className="border-slate-800 bg-slate-900/90 space-y-4">
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800/80">
              ...
              <span className="text-slate-200 font-medium">FastAPI + PennyLane (configured stack)</span>
            </div>
          </div>
        </Card>
          ...
          <div className="space-y-3 text-xs">
            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800/80">
              ...
              <span className="font-mono text-cyan-300 font-semibold">
                {health?.quantum_backend || 'Unavailable until a successful health check'}
              </span>
            </div>
