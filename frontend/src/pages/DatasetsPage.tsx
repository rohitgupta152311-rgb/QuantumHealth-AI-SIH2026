...
export const DatasetsPage: React.FC = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const diseaseId = activeDisease?.id || selectedDiseaseId;
      const res = await uploadDataset(file, diseaseId);
      setUploadResult(res);
      setUploadMessage(
        `Dataset "${res.original_filename || file.name}" uploaded and validated successfully by the server! Ingested ${res.accepted_rows} rows (${res.duplicate_rows} duplicates, ${res.rejected_rows} rejected).`
      );
    } catch (err: any) {
      console.error('Dataset upload error:', err);
      const detail = err.response?.data?.detail;
      if (typeof detail === 'string') {
        setUploadError(`Upload rejected: ${detail}`);
      } else if (Array.isArray(detail)) {
        setUploadError(`Validation failed: ${detail.map((d: any) => d.msg || JSON.stringify(d)).join('; ')}`);
      } else if (detail && typeof detail === 'object') {
        setUploadError(`Validation failed: ${JSON.stringify(detail)}`);
      } else {
        setUploadError(err.message || 'Dataset upload failed. Ensure the CSV headers strictly match the cohort feature schema.');
      }
    } finally {
      setIsUploading(false);
      e.target.value = '';
                    ...
                    {uploadResult && (
                      <>
                        <div className="grid grid-cols-3 gap-2 mt-2 pt-2 border-t border-emerald-500/20 text-[11px] font-mono">
                          <div>
                            <span className="text-emerald-400/70 block">Accepted</span>
                            <span className="font-bold text-emerald-300">{uploadResult.accepted_rows}</span>
                          </div>
                          <div>
                            <span className="text-amber-400/70 block">Duplicates</span>
                            <span className="font-bold text-amber-300">{uploadResult.duplicate_rows}</span>
                          </div>
                          <div>
                            <span className="text-rose-400/70 block">Rejected</span>
                            <span className="font-bold text-rose-300">{uploadResult.rejected_rows}</span>
                          </div>
                        </div>
                        ...
                        {uploadResult.validation_errors && uploadResult.validation_errors.length > 0 && (
                          <div className="mt-2 pt-2 border-t border-emerald-500/20 space-y-1">
                            <span className="text-[10px] uppercase tracking-wider text-rose-300 font-bold block">
                              Row-Level Validation Errors ({uploadResult.validation_errors.length}):
                            </span>
                            <div className="max-h-28 overflow-y-auto space-y-1 text-[10px] font-mono text-slate-300 bg-slate-950 p-2 rounded border border-slate-800">
                              {uploadResult.validation_errors.slice(0, 10).map((err, i) => (
                                <div key={i} className="flex items-start gap-1 text-slate-300">
                                  <span className="text-rose-400 font-semibold">Row {err.row}</span>
                                  <span className="text-slate-500">[{err.column}]:</span>
                                  <span className="text-slate-400 truncate">{err.error}</span>
                                </div>
                              ))}
