                ...
                <table className="w-full text-left text-xs border-collapse font-mono">
                  ...
                  <tbody className="divide-y divide-slate-800/60 text-slate-300">
                    {data.models.map((model, idx) => {
                      const modelName = model.name || model.model_name || `Model-${idx}`;
                      return (
                        <tr key={modelName} className="hover:bg-slate-850/50 transition-colors">
                          <td className="py-3 px-3 font-sans font-semibold text-white flex items-center gap-2">
                            <span className={`w-2 h-2 rounded-full ${
                              modelName.includes('Hybrid')
                                ? 'bg-cyan-400'
                                : modelName.includes('Quantum')
                                ? 'bg-teal-400'
                                : 'bg-sky-400'
                            }`} />
                            {modelName}
