...
def format_bullet_run(p, prefix, text, font_name="Calibri", prefix_size=9.5, text_size=9.5, prefix_color=None, text_color=None, bold_prefix=True):
    p.text = ""
...
def set_header_paragraph(p, text, font_name="Calibri", size=10.8, color=None):
    p.text = ""
    p.alignment = PP_ALIGN.LEFT
    r = p.add_run()
    r.text = text
...
def update_presentation():
    ...
    # Shape 8: Addressing the Problem
    s2_prob = slide2.shapes[8].text_frame
    set_header_paragraph(s2_prob.paragraphs[0], "▪ ADDRESSING THE HEALTHCARE CRISIS", size=10.5)
    s2_prob_bullets = [
    ...
    set_header_paragraph(slide2.shapes[10].text_frame.paragraphs[0], "▪ CLINICAL IMPACT : PRE-SYMPTOMATIC DETECTION (3–5 YRS EARLIER)", size=8.5)

    # Center Column: 4 Solution Pillars (replacing rigid Stage 1-4)
    # Shape 12: Pillar 1 (formerly Stage 1)
    p1 = slide2.shapes[12].text_frame
    set_header_paragraph(p1.paragraphs[0], "▪ PILLAR 1: DATA QUALITY & RELIABILITY GUARD", size=10.2)
    format_bullet_run(p1.paragraphs[1], "• Biomarker Validation : ", "Routine blood biomarkers (Age, BMI, FPG, BP, Lipids, Creatinine) verified at ingestion.", prefix_size=9.2, text_size=9.0)
    format_bullet_run(p1.paragraphs[2], "• Zero Blind Guesses : ", "Sensors out-of-bounds (e.g. FPG=0) trigger instant 2ms safe directive to re-test corrupt labs.", prefix_size=9.2, text_size=9.0)

    # Shape 14: Pillar 2 (formerly Stage 2)
    p2 = slide2.shapes[14].text_frame
    set_header_paragraph(p2.paragraphs[0], "▪ PILLAR 2: 50-30-20 HYBRID ARCHITECTURE", size=10.2)
    format_bullet_run(p2.paragraphs[1], "• Clinical Data Partition : ", "50% Classical Baseline, 30% Quantum Hilbert Training, 20% Unseen Holdout Testing.", prefix_size=9.2, text_size=9.0)
    format_bullet_run(p2.paragraphs[2], "• 99.88% Compression : ", "24 quantum variational angles in 64-D Hilbert space replace 18,000 tree nodes with zero bloat.", prefix_size=9.2, text_size=9.0)

    # Shape 16: Pillar 3 (formerly Stage 3)
    p3 = slide2.shapes[16].text_frame
    set_header_paragraph(p3.paragraphs[0], "▪ PILLAR 3: DUAL-TRACK INFERENCE ENGINE", size=10.2)
    format_bullet_run(p3.paragraphs[1], "• Classical Track : ", "5 high-capacity models (RandomForest, SVM, XGBoost, LR, HistGBM) evaluated concurrently.", prefix_size=9.2, text_size=9.0)
    format_bullet_run(p3.paragraphs[2], "• Quantum Track : ", "6-Qubit VQC with angle re-uploading, 6-simulator support & local SHAP qubit attributions.", prefix_size=9.2, text_size=9.0)

    # Shape 18: Pillar 4 (formerly Stage 4)
    p4 = slide2.shapes[18].text_frame
    set_header_paragraph(p4.paragraphs[0], "▪ PILLAR 4: CALIBRATED CONSENSUS & CDS REPORT", size=10.2)
    format_bullet_run(p4.paragraphs[1], "• Dynamic Brier Fusion : ", "Calibrated weighted voting (default 60C/40Q) with user-configurable quantum weight slider.", prefix_size=9.0, text_size=8.8)
    format_bullet_run(p4.paragraphs[2], "• Cryptographic Proof : ", "100% refusal on severe conflict (|Δ|>0.45); instant 1.1s SHA-256 tamper-evident signed report.", prefix_size=9.0, text_size=8.8)

    # Right Column: Novelty & Advantages
    # Shape 19: 99.88% Hilbert Compression
    s2_r1 = slide2.shapes[19].text_frame
    set_header_paragraph(s2_r1.paragraphs[0], "▪ 1. 99.88% HILBERT COMPRESSION", size=10.5)
    format_bullet_run(s2_r1.paragraphs[1], "• Quantum Compression : ", "24 parameters in 64-D Hilbert space match 18,000 tree nodes — runs on ₹25k laptop with <512MB RAM.", prefix_size=9.4, text_size=9.2)

    # Shape 20: 50-30-20 Data Partition Novelty
    s2_r2 = slide2.shapes[20].text_frame
    set_header_paragraph(s2_r2.paragraphs[0], "▪ 2. 50-30-20 DATA PARTITION NOVELTY", size=10.5)
    format_bullet_run(s2_r2.paragraphs[1], "• Zero Leakage Rigor : ", "50% Classical + 30% Quantum VQC + 20% Unseen Test Split; prevents barren plateaus & overfitting.", prefix_size=9.4, text_size=9.2)

    # Shape 21: 6-Simulator Portability & Safety
    s2_r3 = slide2.shapes[21].text_frame
    set_header_paragraph(s2_r3.paragraphs[0], "▪ 3. 6-SIMULATOR PORTABILITY & SAFETY", size=10.5)
    format_bullet_run(s2_r3.paragraphs[1], "• Universal Portability : ", "Runs across NumPy, PennyLane, Qiskit Aer, Braket & Lightning; 100% safe refusal on conflict (|Δ|>0.45).", prefix_size=9.4, text_size=9.2)

    # Bottom Stat Badges (Shapes 22-25)
    # Shape 22: 211,833 Cohort
    slide2.shapes[22].text_frame.paragraphs[0].runs[0].text = "211,833 Authentic Patients"
    slide2.shapes[22].text_frame.paragraphs[1].runs[0].text = "Dryad / BMJ Open Cohort (Zero Synthetic)"
    # Shape 23: Doctor Validated
    slide2.shapes[23].text_frame.paragraphs[0].runs[0].text = "Doctor-Validated Prototype"
    slide2.shapes[23].text_frame.paragraphs[1].runs[0].text = "Verified Physician Review + Demo Video"
    # Shape 24: Memory & Specs
    slide2.shapes[24].text_frame.paragraphs[0].runs[0].text = "< 512 MB RAM • 1.1s Latency"
    slide2.shapes[24].text_frame.paragraphs[1].runs[0].text = "Dual-Core CPU • Zero GPU • ₹25k Laptop"
    # Shape 25: ICMR-INDIAB
    slide2.shapes[25].text_frame.paragraphs[0].runs[0].text = "ICMR-INDIAB Calibrated"
    slide2.shapes[25].text_frame.paragraphs[1].runs[0].text = "South Asian Phenotype Tuning (BMI ≥ 23)"
    ...
    # Shape 11: Mode 1: 1:1 Clinical OPD
    m1 = slide3.shapes[11].text_frame
    set_header_paragraph(m1.paragraphs[0], "▪ MODE 1: 1:1 CLINICAL OPD (DOCTOR)", size=10.5)
    m1_bullets = [
    ...
    # Shape 12: Mode 2: Village Batch Triage
    m2 = slide3.shapes[12].text_frame
    set_header_paragraph(m2.paragraphs[0], "▪ MODE 2: VILLAGE BATCH TRIAGE (ASHA)", size=10.5)
    m2_bullets = [
    ...
    # Shape 13: Mode 3: Quality Filtration & Reliability Guard
    m3 = slide3.shapes[13].text_frame
    set_header_paragraph(m3.paragraphs[0], "▪ MODE 3: QUALITY FILTRATION & RELIABILITY GUARD", size=10.0)
    m3_bullets = [
    ...
    set_header_paragraph(slide3.shapes[15].text_frame.paragraphs[0], "▪ PRODUCTION TECH STACK & SPECIFICATIONS", size=9.8)
    
    # 8 Tech Items (Shapes 16-23)
    tech_items = [
        (slide3.shapes[16], "PennyLane 6-Qubit VQC", "24 Angles • 64-D Space"),
        (slide3.shapes[17], "Memory: < 512 MB RAM", "Zero GPU (Dual-Core CPU)"),
        (slide3.shapes[18], "Python 3.12 Scientific", "FastAPI Async Engine"),
        (slide3.shapes[19], "Classical Ensembles", "RF(300), SVM, XGB, HistGBM"),
        (slide3.shapes[20], "6 Simulators Supported", "NumPy, Aer, Braket, Lightning"),
        (slide3.shapes[21], "React 18 + TypeScript", "Dynamic Consensus Sliders"),
        (slide3.shapes[22], "Disk Footprint: < 120 MB", "100% Offline Edge Deploy"),
        (slide3.shapes[23], "50-30-20 Split & CIs", "95% Bootstrap Confidence"),
    ]
    for sh, t_bold, t_norm in tech_items:
    ...
    set_header_paragraph(slide3.shapes[25].text_frame.paragraphs[0], "▪ AI ARCHITECTURE: HYBRID QML VS DEEP LEARNING", size=9.0)
    dl_vs_qml = slide3.shapes[27].text_frame
    dl_vs_qml.paragraphs[0].text = ""
    format_bullet_run(dl_vs_qml.paragraphs[0], "• Deep Learning : ", ">10M params, needs 8GB+ GPU (₹10L+ server), opaque black box, fails offline.", prefix_size=7.6, text_size=7.5)
    if len(dl_vs_qml.paragraphs) == 1:
        p_extra = dl_vs_qml.add_paragraph()
    else:
        p_extra = dl_vs_qml.paragraphs[1]
    format_bullet_run(p_extra, "• Hybrid QML : ", "24 params in 64-D space (99.88% compression), 0 GPU (<512MB RAM), calibrated Brier fusion.", prefix_size=7.6, text_size=7.5)

    # Right Bottom Box: System Specifications & Benchmarks
    set_header_paragraph(slide3.shapes[29].text_frame.paragraphs[0], "▪ SYSTEM SPECIFICATIONS & BENCHMARKS", size=9.0)
    s3_bench = slide3.shapes[31].text_frame
    format_bullet_run(s3_bench.paragraphs[0], "⚡ Latency & Speed : ", "1.1s Single OPD  |  12s per 500 Records (~10,000/hr Batch Triage)", prefix_size=7.8, text_size=7.8)
    format_bullet_run(s3_bench.paragraphs[1], "⚡ Hardware & Specs : ", "< 512 MB RAM  |  Zero GPU  |  < 120 MB Disk  |  74.2% QPU Fidelity", prefix_size=7.8, text_size=7.8)
    ...
    set_header_paragraph(s4_pol.paragraphs[0], "▪ NATIONAL POLICY & SCHEME ALIGNMENT", size=9.8)
    s4_pol_bullets = [
    ...
    set_header_paragraph(slide4.shapes[15].text_frame.paragraphs[0], "▪ 3-PHASE ROLLOUT STRATEGY", size=9.8)

    # Shapes 16, 17, 18: Rollout Stages
    p_s1 = slide4.shapes[16].text_frame
    p_s1.paragraphs[0].runs[0].text = "PHASE 1: "
    p_s1.paragraphs[0].runs[1].text = "₹0 Setup"
    p_s1.paragraphs[1].runs[0].text = "PHC Offline Laptops"

    p_s2 = slide4.shapes[17].text_frame
    p_s2.paragraphs[0].runs[0].text = "PHASE 2: "
    p_s2.paragraphs[0].runs[1].text = "State Rollout"
    p_s2.paragraphs[1].runs[0].text = "50-30-20 ASHA Triage"

    p_s3 = slide4.shapes[18].text_frame
    p_s3.paragraphs[0].runs[0].text = "PHASE 3: "
    p_s3.paragraphs[0].runs[1].text = "National Scale"
    p_s3.paragraphs[1].runs[0].text = "₹50,000+ Cr Savings"

    # Shape 19: Medico-Legal Assurance
    s4_med = slide4.shapes[19].text_frame
    set_header_paragraph(s4_med.paragraphs[0], "▪ MEDICO-LEGAL & CLINICAL SAFETY ASSURANCE", size=9.8)
    format_bullet_run(s4_med.paragraphs[1], "• 100% Zero-Hallucination Guard : ", "Hardcoded sentinels safely halt prediction on corrupted sensor inputs (FPG=0).", prefix_size=8.6, text_size=8.6)
    format_bullet_run(s4_med.paragraphs[2], "• Tamper-Evident SHA-256 Hashes : ", "Every CDS PDF report is cryptographically signed; full physician override authority.", prefix_size=8.6, text_size=8.6)

    # Shape 21 & 23: QPU Hardware Specs
    set_header_paragraph(slide4.shapes[21].text_frame.paragraphs[0], "▪ 6-QUBIT QPU SPECIFICATIONS", size=8.6)
    ...
    # Shape 7: Stakeholder Transformation
    s5_stk = slide5.shapes[7].text_frame
    set_header_paragraph(s5_stk.paragraphs[0], "▪ STAKEHOLDER TRANSFORMATION", size=10.2)
    s5_stk_bullets = [
    ...
    set_header_paragraph(s5_c1.paragraphs[0], "▪ 1. CLINICAL CDS RESULTS [SIMPLE → COMPLEX HIERARCHY]", size=8.5)
    format_bullet_run(s5_c1.paragraphs[1], "• Simple Terms (Top) : ", "Risk Tier (Low/Mod/High), Recommended Medical Action, Primary Biomarker Driver.", prefix_size=8.0, text_size=7.8)
    format_bullet_run(s5_c1.paragraphs[2], "• Complex Terms (Below) : ", "Platt-Calibrated Posterior, Born Probability ⟨Z₀⟩, Inter-Model Disagreement Spread (|Δ|).", prefix_size=8.0, text_size=7.8)
    if len(s5_c1.paragraphs) > 3:
        format_bullet_run(s5_c1.paragraphs[3], "• 4 Live Charts : ", "Model Comparison Bar, Feature Waterfall (SHAP), Patient Radar, Agreement Spectrum.", prefix_size=8.0, text_size=7.8)
    else:
        p_c1 = s5_c1.add_paragraph()
        format_bullet_run(p_c1, "• 4 Live Charts : ", "Model Comparison Bar, Feature Waterfall (SHAP), Patient Radar, Agreement Spectrum.", prefix_size=8.0, text_size=7.8)

    # Shape 17: Card 2 (Quantum Novelty & 50-30-20 Test Split)
    s5_c2 = slide5.shapes[17].text_frame
    set_header_paragraph(s5_c2.paragraphs[0], "▪ 2. QUANTUM NOVELTY & TEST SPLIT INSIGHTS", size=8.5)
    format_bullet_run(s5_c2.paragraphs[1], "• 50-30-20 Data Split : ", "50% Classical (5 models), 30% Quantum (6 qubits), 20% Unseen Holdout Testing.", prefix_size=8.0, text_size=7.8)
    format_bullet_run(s5_c2.paragraphs[2], "• Statistical Rigor : ", "95% bootstrap confidence intervals, precision error bands, zero data leakage.", prefix_size=8.0, text_size=7.8)
    if len(s5_c2.paragraphs) > 3:
        format_bullet_run(s5_c2.paragraphs[3], "• Dynamic Ratio Slider : ", "Interactive consensus control (0%–100% quantum weight, default 60C/40Q).", prefix_size=8.0, text_size=7.8)
    else:
        p_c2 = s5_c2.add_paragraph()
        format_bullet_run(p_c2, "• Dynamic Ratio Slider : ", "Interactive consensus control (0%–100% quantum weight, default 60C/40Q).", prefix_size=8.0, text_size=7.8)

    # Shape 20: Card 3 (Operational Use Cases)
    s5_c3 = slide5.shapes[20].text_frame
    set_header_paragraph(s5_c3.paragraphs[0], "▪ 3. OPERATIONAL USE CASES [MASS POPULATION TRIAGE]", size=8.5)
    format_bullet_run(s5_c3.paragraphs[1], "• Use Case 1 (1:1 OPD) : ", "Sub-second (1.1s) second opinion for rural medical officers with signed PDF.", prefix_size=8.0, text_size=7.8)
    format_bullet_run(s5_c3.paragraphs[2], "• Use Case 2 (ASHA Camp) : ", "500 patient records in 12s (~10,000/hr) with prioritized follow-up rosters.", prefix_size=8.0, text_size=7.8)
    if len(s5_c3.paragraphs) > 3:
        format_bullet_run(s5_c3.paragraphs[3], "• Use Case 3 (Epidemiology) : ", "District-level NCD prevalence mapping for National Health Mission.", prefix_size=8.0, text_size=7.8)
    else:
        p_c3 = s5_c3.add_paragraph()
        format_bullet_run(p_c3, "• Use Case 3 (Epidemiology) : ", "District-level NCD prevalence mapping for National Health Mission.", prefix_size=8.0, text_size=7.8)
    ...
    # Shape 7: Scientific Literature
    s6_lit = slide6.shapes[7].text_frame
    set_header_paragraph(s6_lit.paragraphs[0], "▪ SCIENTIFIC LITERATURE & CLINICAL STANDARDS", size=10.8)
    s6_lit_bullets = [
    ...
    # Shape 8: Solutions Already Exist
    s6_comm = slide6.shapes[8].text_frame
    set_header_paragraph(s6_comm.paragraphs[0], "▪ SOLUTIONS ALREADY EXIST (COMMERCIAL SYSTEMS)", size=10.5)
    s6_comm_bullets = [
    ...
    # Shape 9: Our Solution Stands Out
    s6_out = slide6.shapes[9].text_frame
    set_header_paragraph(s6_out.paragraphs[0], "▪ OUR SOLUTION STANDS OUT (QUANTUMHEALTH AI NOVELTY)", size=10.5)
    s6_out_bullets = [
    ...
    # Shape 10: Project Resources & Validation
    s6_res = slide6.shapes[10].text_frame
    set_header_paragraph(s6_res.paragraphs[0], "▪ PROJECT RESOURCES & CLINICAL VALIDATION", size=10.0)
    s6_res_bullets = [
    ...
    # Shape 11: Scan Phone & Trust Guarantee
    s6_qr = slide6.shapes[11].text_frame
    set_header_paragraph(s6_qr.paragraphs[0], "▪ SCAN PHONE • TEST LIVE PROTOTYPE", size=10.2)
    s6_qr_bullets = [
    ...
    prs.save(PPTX_PATH)
    print("Updated PPTX saved successfully to:", PPTX_PATH)
